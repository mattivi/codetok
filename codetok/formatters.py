...
from .parser import FileStats, CODE_EXTENSIONS, DOCUMENTATION_EXTENSIONS, CONFIG_EXTENSIONS
    ...
    def format(self, categories: Dict[str, CategoryStats], output_file: str):
        ...
        report = {
            ...
            'summary': {
                ...
                'total_comments': sum(cat.total_comments for cat in categories.values()),
                ...
                'total_size_bytes': sum(cat.total_size_bytes for cat in categories.values())
            },
            'categories': {}
        }
        ...
        for name, category in categories.items():
            ...
            for file_stat in category.files:
                ...
                ext_name = CODE_EXTENSIONS.get(ext) or DOCUMENTATION_EXTENSIONS.get(ext) or CONFIG_EXTENSIONS.get(ext) or ext
                ...
                'files': [
                    {
                        ...
                        'extension_name': CODE_EXTENSIONS.get(f.extension) or DOCUMENTATION_EXTENSIONS.get(f.extension) or CONFIG_EXTENSIONS.get(f.extension) or f.extension,
...
class ConsoleFormatter:
    ...
    def _print_token_analysis(self, categories: Dict[str, CategoryStats]):
        ...
        Logger.info(f"OpenAI Tokenizer: {Colors.BOLD}cl100k_base{Colors.ENDC} (GPT-4/GPT-3.5-turbo compatible)")
        ...
        for name, category in categories.items():
            if category.total_files > 0:
                percentage = (category.total_tokens / total_tokens * 100) if total_tokens > 0 else 0
                Logger.stat(f"{category.icon} {category.name}", 
    ...
    def _print_detailed_analysis(self, categories: Dict[str, CategoryStats]):
        ...
        if total_lines > 0:
            Logger.stat("Average Tokens per Line", f"{total_tokens / total_lines:.2f}", Icons.CHART)
    ...
    def _print_category_stats(self, category: CategoryStats):
        ...
        Logger.stat("Source Lines (SLOC)", format_number(category.total_sloc), Icons.CODE)
        ...
        # Averages
        if category.total_files > 0:
            Logger.stat("Avg Lines per File", f"{category.avg_lines_per_file:.1f}", Icons.CHART)
            Logger.stat("Avg Tokens per File", f"{category.avg_tokens_per_file:.1f}", Icons.CHART)
