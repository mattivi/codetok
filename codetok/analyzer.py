...
class CodeAnalyzer:
    ...
    def analyze(self) -> Dict[str, CategoryStats]:
        ...
        Logger.info(f"Excluded directories: {', '.join(sorted(list(self.config.exclude_dirs)[:10]))}{'...' if len(self.config.exclude_dirs) > 10 else ''}")
    ...
    def _find_files(self) -> List[Path]:
        ...
        for root, dirs, files in os.walk(base_path):
            ...
            for file in files:
                ...
                if self.config.exclude_patterns and any(fnmatch.fnmatch(file, pattern) for pattern in self.config.exclude_patterns):
                    continue
