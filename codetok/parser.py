...
def count_lines_by_type(content: str, extension: str, filename: str) -> Tuple[int, int, int]:
    """
    ...
    if HAS_PYGMENTS and extension in LEXER_MAPPING:
        try:
            ...
            Logger.warning(f"Pygments error for {filename}: {e}. Falling back to simple detection.")
    ...
    for line in lines:
        ...
        elif extension == '.py' and (stripped.startswith('"""') or stripped.startswith("'''")):
            comment_lines += 1
...
def process_file(file_path: Path) -> FileStats:
    ...
    try:
        ...
        lines_code, lines_comments, lines_blank = count_lines_by_type(content, extension, str(file_path))
...
def get_all_extensions() -> Set[str]:
    ...
    """
    return set(CODE_EXTENSIONS.keys()) | set(DOCUMENTATION_EXTENSIONS.keys()) | set(CONFIG_EXTENSIONS.keys())
